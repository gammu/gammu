...
if [ $# != 0 ];
then
  while getopts "a:b:B:cC:dD:e:f:F:g:G:hJ:K:p:P:r:R:s:S:t:T:u:vx:X:Z" o
  do
    case "$o" in
      "a")
        gcov_arg=$OPTARG
...
then
  ...
  then
    say "    (DEPRECIATE) Learn more at https://github.com/codecov/support/wiki/Merge-Commits"
    # Merge xxx into yyy
...
yaml=$(cd "$proj_root" && git ls-files "*codecov.yml" 2>/dev/null || hg locate "*codecov.yml" 2>/dev/null || find . -name "*codecov.yml")
yaml=$(echo "$yaml" | head -1 | sed -e 's/^\.\///')
...
then
  ...
  say "    ${e}Yaml not found${x}, thats ok!"
  say "    Learn more at https://github.com/codecov/support/wiki/Codecov-Yaml"
...
then
  ...
  then
    ...
    then
      ...
      say "    ${e}->${x} Speed up xcode processing by using use -J 'AppName' (accepting regexp)"
    fi
...
then
  ...
  say "    Please visit https://github.com/codecov/support/wiki/Supported-Languages"
  say "    Choose your language for additional help."
  exit ${exit_with};
fi
...
if [ "$ft_fix" = "1" ];
then
  if [ "$(find "$git_root" -name '*.go' -or -name '*.php' -or -name '*.kt' -or -name '*.swift' -or -name '*.m' -or -name '*.jsx' -or -name '*.cpp' -or -name '*.h' -or -name '*.cxx' -or -name '*.c' -or -name '*.hpp')" != "" ];
  then
...
$(find "$git_root" -type f -not -path '*/vendor/*' -name '*.php' -exec grep -nIH '^[[:space:]]*][[:space:]]*$' {} \;)"
    ...
    then
      adjustments="$adjustments
$(find "$git_root" -type f \( -name '*.h' -or -name '*.cpp' -or -name '*.cxx' -or -name '*.m' -or -name '*.c' -or -name '*.hpp' \) -exec grep -nIH '^[[:space:]]*}' {} \;)
$(find "$git_root" -type f \( -name '*.h' -or -name '*.cpp' -or -name '*.cxx' -or -name '*.m' -or -name '*.c' -or -name '*.hpp' \) -exec grep -nIH '// LCOV_EXCL' {} \;)"
    fi
