...
static GSM_Error SMSDMySQL_Init(GSM_SMSDConfig *Config)
{
	...
	if (!mysql_real_connect(&Config->DB,Config->PC,Config->user,Config->password,Config->database,port,NULL,0)) {
	...
	mysql_free_result(Res);
	mysql_query(&Config->DB,"SET NAMES utf8;");
	return ERR_NONE;
}
...
static GSM_Error SMSDMySQL_InitAfterConnect(GSM_SMSDConfig *Config)
{
	...
	dbgprintf("%s\n",buf);
	if (mysql_real_query(&Config->DB,buf,strlen(buf))) {
		WriteSMSDLog("Error inserting into database (Init): %d %s\n", mysql_errno(&Config->DB), mysql_error(&Config->DB));
		return ERR_UNKNOWN;
	}
...
{
	unsigned char		buffer[10000],buffer2[200],buffer4[10000];
	int 			i,ID;
	MYSQL_RES 		*Res;
	MYSQL_ROW 		Row;
	...
	for (i=0;i<sms->Number;i++) {
		buffer[0]=0;
		if (i==0) {
			sprintf(buffer+strlen(buffer),"INSERT INTO `outbox` (`SenderID`,`DeliveryReport`,`MultiPart`,`InsertIntoDB");
		} else {
			sprintf(buffer+strlen(buffer),"INSERT INTO `outbox_multipart` (`SenderID`,`SequencePosition");
		}
...
{
	unsigned char	buffer[10000],buffer2[200],buff[50];
