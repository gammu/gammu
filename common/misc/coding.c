...
/* Nokia phones add special extension to default alphabet in SMS.
 ...
 */
static unsigned char GSM_NokiaDefaultAlphabetChars[][4] =
{
	{0x1b,0x14,0x00,0x5e},	/* ^	*/
	{0x1b,0x28,0x00,0x7b},	/* {	*/
	{0x1b,0x29,0x00,0x7d},	/* }	*/
...
void DecodeDefault (unsigned char *dest, const unsigned char *src, int len)
{
	...
	for (i = 0; i < len; i++) {
		...
		if (i < (len-1)) {
			j=0;
			while (GSM_NokiaDefaultAlphabetChars[j][0]!=0x00) {
				if (GSM_NokiaDefaultAlphabetChars[j][0]==src[i] && 
				    GSM_NokiaDefaultAlphabetChars[j][1]==src[i+1]) {
					FoundSpecial = true;
					dest[current++] = GSM_NokiaDefaultAlphabetChars[j][2];
					dest[current++] = GSM_NokiaDefaultAlphabetChars[j][3];
					i++;
					break;
				}
				j++;
...
void EncodeDefault(unsigned char *dest, const unsigned char *src, int *len)
{
	...
	for (i = 0; i < *len; i++) {
		...
		j = 0;
		while (GSM_NokiaDefaultAlphabetChars[j][0]!=0x00) {
			if (src[i*2] 	== GSM_NokiaDefaultAlphabetChars[j][2] &&
			    src[i*2+1] 	== GSM_NokiaDefaultAlphabetChars[j][3]) {
				dest[current++] = GSM_NokiaDefaultAlphabetChars[j][0];
				dest[current++] = GSM_NokiaDefaultAlphabetChars[j][1];
				FoundSpecial = true;
				break;
			}
			j++;
