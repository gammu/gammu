...
int MultiBitmapFromPython(PyObject *list, GSM_MultiBitmap *bmp) {
    ...
    if (len > MAX_MULTI_BITMAP) {
        printf("WARNING: truncating Multi Bitmap entries to %d entries! (from %d))\n", MAX_MULTI_BITMAP, len);
        len = MAX_MULTI_BITMAP;
    }
    bmp->Number = len;
    ...
    for (i = 0; i < len ; i++) {
        ...
        if (!PyDict_Check(item)) {
            PyErr_Format(PyExc_ValueError, "Element %i in Bitmaps is not dictionary", i);
            return 0;
        }
