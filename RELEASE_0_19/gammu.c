 ...
 */
static void IncomingUSSD (GSM_StateMachine *s, char *Text) {
    StateMachineObject  *sm;
    int i = 0;
...
StateMachine_init(StateMachineObject *self, PyObject *args, PyObject *kwds)
{
    ...
    if (path != NULL) {
        ...
        if (s != NULL) {
            ...
            /* We ignore error code here... */
            INI_ReadFile(localefile, true, &(self->s.msg));
        }
    }
...
PyMODINIT_FUNC initgammu(void) {
    ...
    if (PyErr_Occurred()) {
        PyErr_Print();
        Py_FatalError("Can not initialize module gammu, see above for reasons");
    }
