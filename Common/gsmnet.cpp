...
char *GSM_GetNetworkName(char *NetworkCode)
{
	...
	EncodeUnicode((unsigned char *)retval,(unsigned char *)"unknown",7);
	while (GSM_Networks[i*2] != NULL) {
		if (!strncmp(GSM_Networks[i*2],NetworkCode,6)) {
			EncodeUnicode((unsigned char *)retval, (unsigned char *)GSM_Networks[i*2+1], strlen(GSM_Networks[i*2+1]));
			break;
		}
		i++;
	}
...
char *GSM_GetCountryName(char *CountryCode)
{
	...
	while (GSM_Countries[i*2] != NULL) {
		if (!strncmp(GSM_Countries[i*2],CountryCode,3)) {
			EncodeUnicode((unsigned char *)retval, (unsigned char *)GSM_Countries[i*2+1], strlen(GSM_Countries[i*2+1]));
			break;
		}
		i++;
	}
