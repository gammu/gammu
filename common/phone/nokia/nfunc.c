...
{
	smprintf(s, "Packing phonebook block with ID = %i, block number = %i, block length = " SIZE_T_FORMAT "\n",id,no+1,size+6);
...
GSM_Error N71_65_AddCalendar2(GSM_StateMachine *s, GSM_CalendarEntry *Note)
{
	...
	GSM_SetCalendarRecurranceRepeat(req+19, NULL, Note);
...
GSM_Error N71_65_AddCalendar1(GSM_StateMachine *s, GSM_CalendarEntry *Note, int *FirstCalendarPos)
{
	...
	case GSM_CAL_MEMO:
		/* byte 12 and 13 */
		GSM_SetCalendarRecurranceRepeat(req+count, NULL, Note);
		count+=2;
...
GSM_Error N71_65_ReplyGetNextCalendar1(GSM_Protocol_Message msg, GSM_StateMachine *s)
{
	...
	case 0x08:
		...
		GSM_GetCalendarRecurranceRepeat(msg.Buffer + 12, NULL, entry);
