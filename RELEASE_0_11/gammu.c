...
#ifdef WITH_THREAD
    PyThread_acquire_lock(AllStateMachinesMutex, 1);
#endif
...
#ifdef WITH_THREAD
    PyThread_release_lock(AllStateMachinesMutex);
#endif
    return 1;
}
...
/* FIXME: can not use python here, Python interaction needs to be moved to END_PHONE_COMM and here we can only store values. This will also allow to use gammu functions from callback, which is nice. */
...
StateMachine_SetAlarm(StateMachineObject *self, PyObject *args, PyObject *kwds) {
    ...
    if (s != NULL) {
        ...
        if (UnicodeLength(gs) > MAX_CALENDAR_TEXT_LENGTH) {
            pyg_warning("Alarm text too long, truncating to %d (from %d)\n", MAX_CALENDAR_TEXT_LENGTH, UnicodeLength(gs));
        }
...
StateMachine_AddCategory(StateMachineObject *self, PyObject *args, PyObject *kwds) {
    ...
    if (UnicodeLength(name) > GSM_MAX_CATEGORY_NAME_LENGTH) {
        pyg_warning("Category name too long, truncating to %d (from %d)\n", GSM_MAX_CATEGORY_NAME_LENGTH, UnicodeLength(name));
