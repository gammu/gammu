...
GSM_Error GSM_DecodeSMSFrameTextEx(GSM_SMSMessage *SMS, unsigned char *buffer, GSM_SMSMessageLayout Layout)
{
	...
	if ((buffer[Layout.TPDCS] & 0x04) == 0x04) SMS->Coding=SMS_Coding_8bit;
	...
	switch (SMS->Coding) {
		...
		//	SMS->Length=buffer[Layout.TPUDL] - (off*8 + w) / 7;
			tmp=GSM_UnpackEightBitsToSeven(w, strlen((char*)buffer+(Layout.Text+off))/*buffer[Layout.TPUDL]-off*/, 160/*SMS->Length*/, buffer+(Layout.Text+off), output);
			dbgprintf("7 bit SMS, length %i\n",SMS->Length);
			SMS->Length = strlen((char*)output);
			DecodeDefault (SMS->Text, output, SMS->Length, true, NULL);
			dbgprintf("%s\n",DecodeUnicodeString(SMS->Text));
			break;
		case SMS_Coding_8bit:
			SMS->Length=((int)buffer[Layout.TPUDL])*2 - off;
		...
		case SMS_Coding_Unicode:
			SMS->Length=(((int)buffer[Layout.TPUDL])*2- off) / 2;
...
GSM_Error GSM_DecodeSMSFrameTextNokia		(GSM_SMSMessage *SMS, unsigned char *buffer, GSM_SMSMessageLayout Layout,int* nUseTextBufferLen)
{
	...
	switch (SMS->Coding) {
		case SMS_Coding_Default:
			...
			SMS->Length=buffer[Layout.TPUDL] - (off*8 + w) / 7;
			tmp=GSM_UnpackEightBitsToSevenEx(w, buffer[Layout.TPUDL]-off, SMS->Length, buffer+(Layout.Text+off), output,nUseTextBufferLen);
			dbgprintf("7 bit SMS, length %i\n",SMS->Length);
			DecodeDefault (SMS->Text, output, SMS->Length, true, NULL);
			dbgprintf("%s\n",DecodeUnicodeString(SMS->Text));
			break;
...
GSM_Error GSM_DecodeSMSFrameStatusReportData(GSM_SMSMessage *SMS, unsigned char *buffer, GSM_SMSMessageLayout Layout)
{
	...
	if (buffer[Layout.TPStatus] < 0x03) {
		EncodeUnicode((unsigned char *)SMS->Text,(unsigned char *)"Delivered",9);
		SMS->Length = 9;      
	} else if (buffer[Layout.TPStatus] & 0x40) {
		EncodeUnicode((unsigned char *)SMS->Text,(unsigned char *)"Failed",6);
		SMS->Length = 6;
	} else if (buffer[Layout.TPStatus] & 0x20) {
		EncodeUnicode((unsigned char *)SMS->Text,(unsigned char *)"Pending",7);
		SMS->Length = 7;
	} else {
		EncodeUnicode(SMS->Text,(unsigned char *)"Unknown",7);
		SMS->Length = 7;
	}
...
GSM_Error GSM_DecodeSMSFrameNokia(GSM_SMSMessage *SMS, unsigned char *buffer, GSM_SMSMessageLayout Layout,int* nUseTextBufferLen)
{
	...
	if (Layout.Text != 255 && Layout.TPDCS!=255 && Layout.TPUDL!=255) {
		GSM_DecodeSMSFrameTextNokia(SMS, buffer, Layout,nUseTextBufferLen);
	}
...
static int GSM_EncodeSMSFrameText(GSM_SMSMessage *SMS, unsigned char *buffer, GSM_SMSMessageLayout Layout)
{
	...
	switch (SMS->Coding) {
		...
		case SMS_Coding_Default:
			...
			EncodeDefault((unsigned char *)buff, SMS->Text, &p, true, NULL);
			size = GSM_PackSevenBitsToEight(w, (unsigned char *)buff, buffer+(Layout.Text+off), p);
			size += off;
			size2 = (off*8 + w) / 7 + p;
...
GSM_Error GSM_EncodeSMSFrame(GSM_SMSMessage *SMS, unsigned char *buffer, GSM_SMSMessageLayout Layout, int *length, bool clear)
{
	...
	/* GSM 03.40 section 9.2.3.10 (TP-Data-Coding-Scheme) and GSM 03.38 section 4 */
	if (Layout.TPDCS != 255) {
		if (SMS->Class>=0 && SMS->Class<5) buffer[Layout.TPDCS] |= (SMS->Class);
		dbgprintf("SMS class %i\n",SMS->Class);
	}
