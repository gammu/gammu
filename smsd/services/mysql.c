...
{
	return SMSDMySQL_Query_Real(Config, query, Config->backend_retries);
}
	...
	for (i = 0; i < sms->Number; i++) {
		if (sms->SMS[i].PDU == SMS_Status_Report) {
			...
			sprintf(buffer, "SELECT ID,Status,UNIX_TIMESTAMP(SendingDateTime),DeliveryDateTime,SMSCNumber FROM `sentitems` WHERE "
				"DeliveryDateTime IS NULL AND " "SenderID='%s' AND TPMR='%i' AND DestinationNumber='%s'", Config->PhoneID, sms->SMS[i].MessageReference, buffer2);
			...
			while ((Row = mysql_fetch_row(Res))) {
				...
				if (strcmp(Row[4], DecodeUnicodeString(sms->SMS[i].SMSC.Number))) {
					if (Config->skipsmscnumber[0] == 0)
						continue;
					if (strcmp(Config->skipsmscnumber, Row[4]))
						continue;
				}
...
static GSM_Error SMSDMySQL_FindOutboxSMS(GSM_MultiSMSMessage * sms, GSM_SMSDConfig * Config, char *ID)
{
	...
	if (SMSDMySQL_Store(Config, "SELECT `ID`,`InsertIntoDB`,`SendingDateTime`,`SenderID` FROM `outbox` WHERE SendingDateTime < NOW() AND SendingTimeOut < NOW() LIMIT 10", &Res)
	    != ERR_NONE) {
	...
	for (i = 1; i < GSM_MAX_MULTI_SMS + 1; i++) {
		if (i == 1) {
			sprintf(buf,
				"SELECT Text,Coding,UDH,Class,TextDecoded,ID,DestinationNumber,MultiPart,RelativeValidity,DeliveryReport,CreatorID FROM `outbox` WHERE ID='%s'",
				ID);
		} else {
...
static GSM_Error SMSDMySQL_MoveSMS(GSM_MultiSMSMessage * sms UNUSED, GSM_SMSDConfig * Config, char *ID, gboolean alwaysDelete UNUSED, gboolean sent UNUSED)
{
	char buffer[100];
...
static GSM_Error SMSDMySQL_AddSentSMSInfo(GSM_MultiSMSMessage * sms, GSM_SMSDConfig * Config, char *ID, int Part, GSM_SMSDSendingError err, int TPMR)
{
	...
	switch (sms->SMS[Part - 1].Coding) {
		...
		case SMS_Coding_Default_No_Compression:
			...
			break;
		case SMS_Coding_8bit:
			EncodeHexBin(query + query_pos, sms->SMS[Part - 1].Text, sms->SMS[Part - 1].Length);
			query_pos = strlen(query);
		default:
			break;
	}
	query_pos += snprintf(query + query_pos, sizeof(query) - query_pos, "','%s','", DecodeUnicodeString(sms->SMS[Part - 1].Number));
	switch (sms->SMS[Part - 1].Coding) {
		case SMS_Coding_Unicode_No_Compression:
	...
	if (sms->SMS[Part - 1].UDH.Type != UDH_NoUDH) {
		EncodeHexBin(query + query_pos, sms->SMS[Part - 1].UDH.Text, sms->SMS[Part - 1].UDH.Length);
		query_pos = strlen(query);
	}
	query_pos += snprintf(query + query_pos, sizeof(query) - query_pos, "','%i','", sms->SMS[Part - 1].Class);
	switch (sms->SMS[Part - 1].Coding) {
		case SMS_Coding_Unicode_No_Compression:
		case SMS_Coding_Default_No_Compression:
			EncodeUTF8(utf_text, sms->SMS[Part - 1].Text);
			mysql_real_escape_string(&Config->DBConnMySQL, escaped_text, utf_text, strlen(utf_text));
			query_pos += snprintf(query + query_pos, sizeof(query) - query_pos, "%s", escaped_text);
			break;
		default:
			break;
	}
	query_pos += snprintf(query + query_pos, sizeof(query) - query_pos, "','%s','", Config->DT);
	if (sms->SMS[Part - 1].SMSC.Validity.Format == SMS_Validity_RelativeFormat) {
		query_pos += snprintf(query + query_pos, sizeof(query) - query_pos, "%i')", sms->SMS[Part - 1].SMSC.Validity.Relative);
	} else {
		query_pos += snprintf(query + query_pos, sizeof(query) - query_pos, "-1')");
	}
	if (SMSDMySQL_Query(Config, query) != ERR_NONE) {
