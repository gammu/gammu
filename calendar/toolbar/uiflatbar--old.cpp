...
void CMyToolBar::SetButtonImage(LPCTSTR pBtnFile, int nCount)
{
	...
	if(pBtnFile)
	{
		...
		for(int i = 0; i < 4; i ++)
		{
			CString sBtnImg = sAppPath + "skin\\default\\Organize\\" + pBtnFile + sBtnFn[i];
...
void CMyToolBar::SetImgBk(LPCSTR lpszPath, bool bUseBK)
{
	SAFE_DELPTR(m_pImgBk);
...
void CMyToolBar::SetToolTipText(LPCTSTR pText,int nToolID, BOOL bActivate)
{
	...
	if (m_ToolTip.GetToolCount() == 0)
	{
		...
		m_ToolTip.AddTool(this, pText, rectBtn, nToolID);
	}

	// Set text for tooltip
	m_ToolTip.UpdateTipText(pText, this, nToolID);
