...
BOOL CSearchDlg::OnInitDialog() 
{
	...
	//string sProfile = sSkin + string("Organize\\search.ini");
	Tstring sProfile = sSkin + Tstring(_T("Calendar\\search.ini"));//nono, 2004_1101
	...
	CRect rect;
	if(al_GetSettingRect(_T("title"),_T("rect"),const_cast<TCHAR*>(sProfile.c_str()),rect))
		m_scTitle.MoveWindow(rect);

	COLORREF color;
	if(al_GetSettingColor(_T("panel"),_T("brush"),const_cast<TCHAR*>(sProfile.c_str()),color))
		m_scTitle.SetBrushColor(color);
	
	TCHAR szTxt[MAX_PATH];
	if(al_GetSettingString(_T("public"),_T("IDS_ENTERNAME"),theApp.m_szRes,szTxt))
		m_scTitle.SetWindowText(szTxt);
	//load combo box
...
void CSearchDlg::OnBnFindnext() 
{
	...
	if(m_sOldTextl != Tstring(szText)){
		m_iStart = 0;
		m_sOldTextl = Tstring(szText);
	}
	else if(m_iStart != -1)
		m_iStart ++;
