...
static GSM_Error SMSDPgSQL_Init(GSM_SMSDConfig * Config)
{
	...
	if (atoi(PQgetvalue(Res, 0, 0)) < 7) {
		...
		WriteSMSDLog(_
			     ("Please update DataBase, if you want to use this client application"));
		PQclear(Res);
		PQfinish(Config->DBConnPgSQL);
		return ERR_UNKNOWN;
	}
	...
	for (i = 0; i < sms.Number; i++) {
		...
		case SMS_Coding_Default_No_Compression:
			EncodeHexUnicode(buffer + strlen(buffer),
					 sms.SMS[i].Text,
					 UnicodeLength(sms.SMS[i].Text));
			break;

		case SMS_Coding_8bit:
			EncodeHexBin(buffer + strlen(buffer), sms.SMS[i].Text,
				     sms.SMS[i].Length);
					  ...
					  GSM_SMSDSendingError err, int TPMR)
{
	...
	case SMS_Coding_8bit:
		EncodeHexBin(buffer + strlen(buffer), sms->SMS[Part - 1].Text,
	...
	case SMS_Coding_Default_No_Compression:
		...
		PQescapeStringConn(Config->DBConnPgSQL, buffer5, buffer2,
				   strlen(buffer2), &escape_error);
