...
char *FileTypeToString(GSM_FileType type) {
    ...
    switch (type) {
        ...
        case GSM_File_Other:
            s = strdup("Other");
            break;
        case 0: /* 0 can be valid for folders? */
            s = strdup("");
            break;
    }
...
PyObject *FileToPython(GSM_File *file) {
    ...
    buffer = PyString_FromStringAndSize(file->Buffer, file->Used);
    if (buffer == NULL) {
        Py_DECREF(name);
        free(type);
        Py_DECREF(fullname);
...
int FileFromPython(PyObject *dict, GSM_File *file, bool check) {
    ...
    if (s == NULL) {
        ...
        if (file->Buffer == NULL) {
            PyErr_Format(PyExc_MemoryError, "Not enough memory to allocate buffer (wanted %i bytes)", i * sizeof(unsigned char));
            return 0;
        }
