// sonyericssonatdlg.cpp : implementation file ( refer to motodlg
//

#include "stdafx.h"
#include "..\phonebook.h"
#include "sonyericssonatdlg.h"
...
CSonyEricssonAtDlg::CSonyEricssonAtDlg(CWnd* pParent /*=NULL*/)
	: CBaseDlg(CSonyEricssonAtDlg::IDD, pParent)
{
	//{{AFX_DATA_INIT(CSonyEricssonAtDlg)
	//}}AFX_DATA_INIT
	m_hFont = NULL;
}
CSonyEricssonAtDlg::~CSonyEricssonAtDlg()
{
	if(m_hFont)
...
void CSonyEricssonAtDlg::DoDataExchange(CDataExchange* pDX)
{
	CBaseDlg::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CSonyEricssonAtDlg)
...
BEGIN_MESSAGE_MAP(CSonyEricssonAtDlg, CBaseDlg)
	//{{AFX_MSG_MAP(CSonyEricssonAtDlg)
...
/////////////////////////////////////////////////////////////////////////////
// CSonyEricssonAtDlg message handlers

BOOL CSonyEricssonAtDlg::OnInitDialog() 
{
	...
	//Load static
	CStaticEx	*pStaticEx[4] = {&m_scName,&m_scType,&m_scData,&m_scStorage};
	TCHAR szScSec[4][32] = {_T("name_sc"),_T("type_sc"),_T("data_sc"),_T("storage_sc")};
	TCHAR szScID[4][32] = {_T("IDS_A_NAME"),_T("IDS_M_TYPE"),_T("IDS_M_DATA"),_T("IDS_A_LOCATION")};
	for(int i = 0 ; i < 4 ; i ++)
	...
	//new Control ******************************************* ++++
	CStaticEx	*pStaticEx2[9] = {&m_scGroup,&m_scNickName,&m_scBirthday,&m_scStreet1,&m_scStreet2,&m_scCity,&m_scState,&m_scZip,&m_scCountry};
	TCHAR szScSec2[9][32] = {_T("group_sc"),_T("nickname_sc"),_T("birthday_sc"),_T("street1_sc"),_T("street2_sc"),_T("city_sc"),_T("state_sc"),_T("zip_sc"),_T("country_sc")};
	TCHAR szScID2[9][32] = {_T("IDS_A_GROUP"),_T("IDS_S_NICKNAME"),_T("IDS_A_BIRTHDAY"),_T("IDS_M_STREET1"),_T("IDS_M_STREET2"),_T("IDS_CITY"),_T("IDS_SE_STATE"),_T("IDS_ZIP"),_T("IDS_COUNTRY")};

//	if(strcmp(theApp.m_szPhone,"E1000") == 0)
//	{
//		wsprintf(szScID2[2],"%s","IDS_M_INFO");// IDS_M_STREET1
//		wsprintf(szScID2[3],"%s","IDS_A_ADDRESS");// IDS_M_STREET2
//	}
	...
	CWnd *pEDWnd[9] = {&m_cbGroup,&m_edNickName,&m_edZip,&m_edStreet1,&m_edStreet2,&m_edState,&m_edCountry,&m_edCity,&m_dpBirthday};
	TCHAR szEDWndSec[9][32] = {_T("group_cb"),_T("nickname_ed"),_T("zip_ed"),_T("street1_ed"),_T("street2_ed"),_T("state_ed"),_T("country_ed"),_T("city_ed"),_T("birthday_dp")};
	for(i = 0 ; i < 9 ; i ++){
		CRect rect;
	...
	CWnd *pWnd[3] = {&m_edName,&m_cbType,&m_edData};
	TCHAR szWndSec[3][32] = {_T("name_ed"),_T("type_cb"),_T("data_ed")};
	for(i = 0 ; i < 3 ; i ++){
		CRect rect;
...
void CSonyEricssonAtDlg::OnRdSim() 
{
...
void CSonyEricssonAtDlg::OnRdMobile() 
{
...
void CSonyEricssonAtDlg::OnSelchangeCbType() 
{
...
void CSonyEricssonAtDlg::OnCancel() 
{
...
void CSonyEricssonAtDlg::xEnableControl(int iState)
{
...
CSonyEricssonAtDlg::xSetComboItem()
{
	...
	//set type item
	TCHAR szType[3][32]={_T("IDS_M_HOME"),_T("IDS_M_COMPANY"),_T("IDS_M_MOBILE")};
	for(int i = 0 ; i < 3 ; i ++){
...
//	if(strcmp(theApp.m_szPhone,"E1000") == 0)
//	{
//		if(al_GetSettingString("public","IDS_M_VIDEO",theApp.m_szRes,szTxt))
//			m_cbType.AddString(szTxt);
//	}
	m_cbType.SetCurSel(0);
...
CSonyEricssonAtDlg::xSetDataToDlgCtrl()
{
	if(m_pData){
		//get data deq
...
BOOL CSonyEricssonAtDlg::xCheckData()
{
	CString str;
	TCHAR szText[MAX_PATH];
...
void CSonyEricssonAtDlg::OnChBirthday() 
{
