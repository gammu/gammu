...
static bool PHONE_IsPointBitmap(GSM_Phone_Bitmap_Types Type,unsigned char *buffer, int x, int y, int width, int height)
{
	int i=0, pixel;
...
void PHONE_DecodeBitmap(GSM_Phone_Bitmap_Types Type,unsigned char *buffer, GSM_Bitmap *Bitmap)
{
...
static GSM_Error loadgif(FILE *file, GSM_MultiBitmap *bitmap)
{
	...
	bmap->BinaryPic.Buffer = buffer = (unsigned char  *)malloc(length);
	if (bmap->BinaryPic.Buffer == NULL)
		return ERR_MOREMEMORY;

	fread(buffer, 1, length, file);
        dbgprintf("Length %i name \"%s\"\n", length,
		DecodeUnicodeString((unsigned char *)bmap->Name));
...
GSM_Error GSM_ReadBitmapFile(char *FileName, GSM_MultiBitmap *bitmap)
{
	...
	bitmap->Bitmap[0].Name = (char *)malloc((strlen(FileName) + 1) * 2);
	if (bitmap->Bitmap[0].Name == NULL)
		return ERR_MOREMEMORY;
	EncodeUnicode((unsigned char *)bitmap->Bitmap[0].Name, (unsigned char *)FileName, strlen(FileName));
