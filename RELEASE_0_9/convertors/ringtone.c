...
PyObject *RingtoneToPython(GSM_Ringtone *inring) {
    ...
    if (inring->Format != RING_NOTETONE) {
        if (GSM_RingtoneConvert(&ring, inring, RING_NOTETONE) != ERR_NONE) {
            printf("Warning: ringtone can not be converted to RING_NOTETONE, ignoring!\n");
            Py_INCREF(Py_None);
            return Py_None;
        }
    } else {
...
int RingtoneFromPython(PyObject *dict, GSM_Ringtone *ring) {
    ...
    if (len > MAX_RINGTONE_NOTES) {
        printf("WARNING: truncating Notes entries to %d entries! (from %d))\n", MAX_RINGTONE_NOTES, len);
        len = MAX_RINGTONE_NOTES;
    }
