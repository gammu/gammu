 ...
 */
static StateMachineObject *FindStateMachine(const GSM_StateMachine *s) {
    int i = 0;
...
StateMachine_Terminate(StateMachineObject *self, PyObject *args, PyObject *kwds)
{
    ...
    self->s.Phone.Functions->SetIncomingSMS(&(self->s), false);
    self->s.Phone.Functions->SetIncomingCall(&(self->s), false);
    self->s.Phone.Functions->SetIncomingCB(&(self->s), false);
    self->s.Phone.Functions->SetIncomingUSSD(&(self->s), false);
...
StateMachine_DialService(StateMachineObject *self, PyObject *args, PyObject *kwds) {
    ...
    BEGIN_PHONE_COMM
    error = self->s.Phone.Functions->DialService(&(self->s), s);
    END_PHONE_COMM
...
StateMachine_SendFilePart(StateMachineObject *self, PyObject *args, PyObject *kwds) {
    ...
    BEGIN_PHONE_COMM
    error = self->s.Phone.Functions->SendFilePart(&(self->s), &File, &pos, &handle);
    END_PHONE_COMM
