...
GSM_Error WINAPI myinitialize  (int *phone,
				char *device,
				char *connection,
				char *model,
				char *logfile,
				...
				void (*SMSCallBack) 	 (int s))
{
	...
	*phone = 0;
	...
	s[*phone].s.CFGDebugFile	= logfile;
	strcpy(s[*phone].s.CFGDebugLevel,logfiletype);
	s[*phone].s.CFGLockDevice	= "";
	s[*phone].s.CFGDevice		= device;
	s[*phone].s.CFGConnection	= connection;
	strcpy(s[*phone].s.CFGModel,"");
...
GSM_Error WINAPI mydetectdevice(char *device,
				char *connection,
				char *logfile,
				char *logfiletype)

{
	...
	s[0].s.CFGLocalize 		= "";
	s[0].s.CFGSyncTime		= "";
	s[0].s.CFGDebugFile		= logfile;
	strcpy(s[0].s.CFGDebugLevel,logfiletype);
	s[0].s.CFGLockDevice		= "";
	s[0].s.CFGDevice		= device;
	strcpy(s[0].s.CFGModel,"");

	s[0].s.CFGConnection = "at115200";
	error=GSM_InitConnection(&s[0].s,2);
	switch (error) {
	case GE_NONE:
		error2 = GSM_TerminateConnection(&s[0].s);
		strcpy(connection,"at115200");
		return error2;
	...
	s[0].s.CFGConnection = "at19200";
	error=GSM_InitConnection(&s[0].s,2);
	switch (error) {
	case GE_NONE:
		error2 = GSM_TerminateConnection(&s[0].s);
		strcpy(connection,"at19200");
		return error2;
	case GE_DEVICEOPENERROR:
		return error;
	default:
	...
	s[0].s.CFGConnection = "fbus";
	error=GSM_InitConnection(&s[0].s,2);
	switch (error) {
	case GE_NONE:
		error2 = GSM_TerminateConnection(&s[0].s);
		strcpy(connection,"fbus");
		return error2;
	case GE_DEVICEOPENERROR:
		return error;
	default:
                       ...
                       LPVOID lpReserved )
{
	...
	switch (ul_reason_for_call) {
		case DLL_PROCESS_ATTACH:
			for (i=0;i<10;i++) s[i].s.opened = false;
			break;
