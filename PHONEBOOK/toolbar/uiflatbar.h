...
class CTRL_EXT_CLASS CMyToolBar : public CFlatToolBar 
{
	DECLARE_DYNAMIC(CMyToolBar)
public:
	void UninitMemDC();
	void InitMemDC();
	void SetImgBk(LPCTSTR lpszPath, bool bUseBK = true);
	void SetColorBk(COLORREF &cr);
	void SetImgBk(CImage *pImg,bool bUseBK = true);
	CMyToolBar();
	~CMyToolBar();

	void SetButtonBk(LPCTSTR pBkFile);
	void SetButtonImage(LPCTSTR pBtnFile, int nCount);
	void SetUSeBk(bool bFlag) { m_bUseBkImg = bFlag;}

	void InitToolTip();
	void SetToolTipText(LPCTSTR pText,int nToolID, BOOL bActivate = TRUE);
	void SetToolTipText(int nId,int nToolID, BOOL bActivate = TRUE);
	void ActivateTooltip(BOOL bActivate = TRUE);

    void SendMegToToolTip(UINT message,UINT nFlags,CPoint point); 
	CDC m_memDC[5];
	CBitmap		m_bitmap[5];		// Offscreen bitmap
...
private:
	CImageArray *m_pBtnBk;
	CImageArray *m_pBtnIcon[4];		// (0)Normal, (1)Down, (2)Highlight, (3)Gray
	CImage	*m_pImgBk;		
	int m_nBtnCount;
	bool m_bUseBkImg;
	COLORREF m_crBk;
